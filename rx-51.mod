...
xkb_symbols "english_base" {
    ...
    // 1. row
    key <AD01>	{ [	q,		Q,		1,		yen		] };
    key <AD02>	{ [	w,		W,		2,		twosuperior		] };
    key <AD03>	{ [	e,		E,		3,		threesuperior	] };
    key <AD04>	{ [	r,		R,		4,		onequarter		] };
    key <AD05>	{ [	t,		T,		5,		onehalf			] };
    key <AD06>	{ [	y,		Y,		6,		threequarters	] };
    key <AD07>	{ [	u,		U,		7,		mu			] };
    key <AD08>	{ [	i,		I,		8,		multiply	] };
    key <AD09>	{ [	o,		O,		9,		division	] };
    key <AD10>	{ [	p,		P,		0,		guillemotright	] };
    key <AB08>	{ [	comma,		semicolon,	equal,		equal		] };

    // 2. row
    key <AC01>	{ [	a,		A,		asterisk,	degree			] };
    key <AC02>	{ [	s,		S,		plus,		plusminus		] };
    key <AC03>	{ [	d,		D,		numbersign,	asciicircum		] };
    key <AC04>	{ [	f,		F,		minus,		periodcentered	] };
    key <AC05>	{ [	g,		G,		underscore,	overbar			] };
    key <AC06>	{ [	h,		H,		parenleft,	cent			] };
    key <AC07>	{ [	j,		J,		parenright,	bracketleft		] };
    key <AC08>	{ [	k,		K,		ampersand,	bracketright	] };
    key <AC09>	{ [	l,		L,		exclam,		exclamdown		] };
    key <AB09>	{ [	period,		colon,		question,	questiondown	] };

    // 3. row
    key <AB01>	{ [	z,		Z,		asciitilde,	braceleft		] };
    key <AB02>	{ [	x,		X,		dollar,		braceright		] };
    key <AB03>	{ [	c,		C,		bar,		brokenbar		] };
...
xkb_symbols "arrows_4btns" {
    ...
    // normal 2nd level must not be enumerated to allow text selection with Shift key
    key <UP>	{ type[Group1] = "PC_FN_LEVEL2", symbols[Group1] = [	Up, Home	] };
    key <LEFT>	{ type[Group1] = "PC_FN_LEVEL2", symbols[Group1] = [	Left, Escape	] };
    key <DOWN>	{ type[Group1] = "PC_FN_LEVEL2", symbols[Group1] = [	Down, End	] };
